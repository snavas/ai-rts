 ...
 * @author Sandra
 */
public class LRTA_C extends LRTA{

    private Node inicio;
    private List<Node> objetivos;
    private Node[][] worldGraph;

    public LRTA_C(Node inicio, List<Node> objetivos, Node[][] worldGraph) {
        this.inicio = inicio;
    ...
    private double heuristic(Node u) {
        // DISTANCIA Chebyshev
        return Math.max(Math.abs(u.getX() - objetivos.get(0).getX()), Math.abs(u.getY() - objetivos.get(0).getY()));     
    }
