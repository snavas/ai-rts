 ...
 * @author Sandra
 */
public class LRTA_E extends LRTA{

    private Node inicio;
    private List<Node> objetivos;
    private Node[][] worldGraph;

    public LRTA_E(Node inicio, List<Node> objetivos, Node[][] worldGraph) {
        this.inicio = inicio;
    ...
    private double heuristic(Node u) {
        // DISTANCIA EUCLIDEA
        return Math.sqrt(Math.pow(u.getX() - objetivos.get(0).getX(),2) + Math.pow(u.getY() - objetivos.get(0).getY(),2));
    }
